...
export function ProjectCardTwo({ project }: { project: any }) {
  ...
  return (
    ...
    >
      ...
      <p className="pointer-events-none relative z-40 mt-6 flex items-center text-sm font-medium text-zinc-400 transition group-hover:-translate-y-0.5 group-hover:text-[#50ffaf] dark:text-zinc-200 dark:group-hover:text-[#50ffaf]">
        <span className="mr-2">{url}</span>
        <Icons.externalLinkIcon className="h-4 w-4 flex-none" />
      </p>
      ...
      <AnimatePresence>
        {isHovering && (
          <motion.footer
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-px rounded-2xl border border-dashed border-zinc-900/30 dark:border-zinc-100/20" />
